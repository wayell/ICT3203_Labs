...
(async function runTest() {
    ...
    const junitReport = create('testsuite')
        .att('name', 'Forget Password Page Test')
        .ele('properties');
    ...
    try {
        ...
        const linkElement = await driver.findElement(By.linkText("Forgot your Password?"));
        await linkElement.click();

        const sign_up_page = await driver.findElement(By.id('forget_button'));
        if (sign_up_page) {
            console.log('Forget password page renders successfully.');
        } else {
            console.log('Forget password page not renders successfully.');
            junitReport.ele('testcase', { name: 'Forget password Page Rendering', status: 'failed' })
                .ele('error', { message: 'Forget Password page not rendered successfully' });
        }
        ...
        const reportFilePath = path.join(__dirname, 'junit-password-report.xml');
